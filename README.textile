...
(youtube and vimeo additionally has @media_thumbnail_large@, which is the HQ version of the thumbnail)
...
Muri recently added the ability to fetch media information, leveraging the media service API's
There are 3 ways to utilize this functionality:
# Can call fetch! directly on a muri parsed object
    (Calling fetch! on a muri object returns a boolean indicating if the API could fetch information)
# Can call new_muri_object = Muri.fetch(parsed_muri_object)
# Can call muri_object = Muri.parse_and_fetch('www.blahblah.com')
As may be expected, this introduces a slew of new attributes which can be returned from @media_.....@ method calls.

Here is a list of the currently available attributes:
* media_title
* media_description
* media_posted
* media_updated
